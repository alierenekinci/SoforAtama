...
def engine_optimize(response):
    ...
    sofor_i = len(soforler)
    otobus_j = len(otobusler)
    hat_k = len(hatlar)
    sefer_l = len(seferler)
    ...
    # Her şoförün her gün için her otobüs için her hat için her sefer için bir değişken oluşturulması
    for i in soforler:
        for j in otobusler:
    ...
    # Her seferin sadece bir şoför tarafından yapılması kısıtı
    for y in gun_sayisi_y:
        for k in hatlar:
            for l in seferler:
    ...
    for i in soforler:
        ...
        for j in otobusler:
            for y in gun_sayisi_y:
                for k in hatlar:
                    for l in seferler:
                        shifts_worked.append(X[(i["sofor_id"], j["otobus_id"], y, k["hat_id"], l["sefer_id"])])
